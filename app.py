...
# --- 2. IMPROVED CSS (Scoped only to Navigation) ---
st.markdown("""
    <style>
    /* target only buttons inside the navigation data-testid */
    ...
    /* Fixed height for navigation area with scrolling */
    .nav-scroll {
        max-height: 120px;
        overflow-y: auto;
        padding: 10px;
...
if role == "✍️ Student":
    ...
    if 'exam_active' not in st.session_state and 'final_score' not in st.session_state:
        ...
        if df is not None:
            ...
            if st.button("🚀 START EXAM", key="start_btn_global", use_container_width=True):
        ...
        st.write("### 🧭 Navigation")
        with st.container():
            # Create a div that handles scrolling for the question grid
            st.markdown('<div class="nav-scroll">', unsafe_allow_html=True)
                ...
                for c in range(10):
                    ...
                    if idx < len(q_df):
                        with cols[c]:
                            ...
                            # Using secondary kind for smaller CSS targeting
        ...
        if 'expiry_time' in st.session_state:
            ...
            # Standard buttons here will remain normal size
            if curr > 0: b1.button("⬅️ Previous", on_click=lambda: st.session_state.update({"current_q": curr-1}), use_container_width=True)
